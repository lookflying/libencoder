...
extern int init_encoder(int argc, char *argv[]);
extern int encode_frame(unsigned char *inputdata,
		struct coded_buff *avc_p);
extern int close_encoder();
...
static int intra_period = 16; /*SFW 30->3*/
...
static void 
upload_yuv_to_surface(unsigned char *yuv_p, VASurfaceID surface_id);
...
upload_thread_function(void *data)
{
	...
	upload_yuv_to_surface(param->yuv_p, param->surface_id);

	return NULL;
}

static void	alloc_encode_resource_lib()
{
	VAStatus va_status;
...
static void upload_yuv_to_surface(unsigned char *yuv_p, VASurfaceID surface_id)
{
	...
	y_src = yuv_p;
	u_src = yuv_p + y_size; /* UV offset for NV12 */
	v_src = yuv_p + y_size + u_size;
...
static int begin_picture(int frame_num, int display_num, int slice_type, int is_idr)
{
	VAStatus va_status;
	...
	/*if (frame_num == 0) SFW if(slice_type == SLICE_TYPE_I)*/
	...
	if (frame_bit_rate > 0) {
		misc_hrd_param->initial_buffer_fullness = frame_bit_rate * 1024 / 2;
		misc_hrd_param->buffer_size = frame_bit_rate * 1024;
	} else {
...
static void sps_rbsp(bitstream *bs)
{
	...
	if ( frame_bit_rate < 0 ) {
		...
		{
			bitstream_put_ui(bs, 600, 32);
			bitstream_put_ui(bs, 30000, 32);
			bitstream_put_ui(bs, 1, 1);
		}
...
build_nal_slice(FILE *avc_fp, int frame_num, int display_frame, int slice_type, int is_idr)
{
	...
	static int
store_coded_buffer(struct coded_buff *avc_p, int slice_type)
{
	VACodedBufferSegment *coded_buffer_segment;
	unsigned char *coded_mem;
	...
	memcpy(avc_p->buff,coded_mem,slice_data_length);
	avc_p->length=slice_data_length;
	...
	static void
encode_picture(struct coded_buff *avc_p,
		...
		int next_display_num)
{
	...
	begin_picture(frame_num, display_num, slice_type, is_idr);
	...
	do {
		...
		ret = store_coded_buffer(avc_p, slice_type);
	} while (ret);
...

{
	...
	seq_param->time_scale = 30000;
	seq_param->num_units_in_tick = 600;			/* Tc = num_units_in_tick / time_sacle */
	...
	seq_param->seq_fields.bits.direct_8x8_inference_flag = 1; /*SFW 0->1*/

	seq_param->seq_fields.bits.log2_max_frame_num_minus4 = 4;  /*SFW  0->4*/
	seq_param->seq_fields.bits.log2_max_pic_order_cnt_lsb_minus4 = 1;/*SFW  2->1*/
...
int init_encoder(int argc, char *argv[])
{
	...
	if (argc == 4 || argc == 5) {
		qp_value = -1;
		sscanf(argv[3], "qp=%d", &qp_value);
		if ( qp_value == -1 ) {
			frame_bit_rate = -1;
			sscanf(argv[3], "fb=%d", &frame_bit_rate);
			if (  frame_bit_rate == -1 ) {
				show_help();
				return -1;
			}
	...
	if (argc == 5) {
		sscanf(argv[4], "mode=%d", &mode_value);
		if ( mode_value == 0 ) {
			i_frame_only = 1;
			i_p_frame_only = 0;
		}
	...
	/*HACK fixed frame bitrate*/
	frame_bit_rate=6000;
	printf("width=%d,height=%d,framebitrate=%d\n",picture_width,picture_height,frame_bit_rate);
	avcenc_context_init(picture_width, picture_height);
	create_encode_pipe();
	alloc_encode_resource_lib();
		...
		struct coded_buff *avc_p)
{
	 ...
	 */
	if ( i_frame_only ) {
		encode_picture(avc_p,enc_frame_number, frame_number, frame_number==0, SLICE_TYPE_I, 0, frame_number+1);
		frame_number++;
		enc_frame_number++;
		...
		if ( (frame_number % intra_period) == 0 ) {
			encode_picture(avc_p,enc_frame_number, frame_number, frame_number==0, SLICE_TYPE_I, 0, frame_number+1);
			frame_number++;
			if (enc_frame_number == 30)
			{
				intra_period = 100;
			}
			enc_frame_number++;
		} else {
			encode_picture(avc_p,enc_frame_number, frame_number, frame_number==0, SLICE_TYPE_P, 0, frame_number+1);
			frame_number++;
			enc_frame_number++;
		}
	}
...
#if DUMP
	if (!opened){
		yuvFile = fopen ("/dev/shm/yuv.dump","ab+");
		h264File = fopen ("/dev/shm/h264.dump","ab+");
		opened = true;
	}
	if (yuvFile != NULL && h264File !=NULL){
		fwrite ((char*)inputdata , sizeof(char), picture_width*picture_height*3/2, yuvFile);
		fwrite ((char*)avc_p->buff , sizeof(char), avc_p->length, h264File);
	}
#endif
...
/*
	 ...
	 yuv_fp = fopen(argv[1],"rb");
	 if ( yuv_fp == NULL){
	 printf("Can't open input YUV file\n");
	 return -1;
	 }

	 avc_fp = fopen(argv[2], "wb");	
	 if ( avc_fp == NULL) {
	 fclose(yuv_fp);
	 ...
	 cdbf.buff=(unsigned char*)malloc(width*height*3);
	 unsigned char* inputyuv=(unsigned char*)malloc(width*height*3/2);
	 sprintf(mkarg[parn],"main\0");
	 parg[parn]=mkarg[parn];
	 parn++;
	 sprintf(mkarg[parn],"%d\0",width);
	 parg[parn]=mkarg[parn];
	 parn++;
	 sprintf(mkarg[parn],"%d\0",height);
	 parg[parn]=mkarg[parn];
	 parn++;

	 sprintf(mkarg[parn],"fb=1000\0");
	 parg[parn]=mkarg[parn];
	 parn++;
	 sprintf(mkarg[parn],"mode=1\0");
	 parg[parn]=mkarg[parn];
	 parn++;

	 if(!init_encoder(parn,parg))
	 {
	 printf("Init error.\n");
	 return 1;
	 }
	 ...
	 rtpParams rtpparams;
	 char strip[]="172.16.180.70";
	 unsigned int destip,hostbyteip;
	 destip=inet_addr(strip);
	 hostbyteip=ntohl(destip);

	 rtpparams.DestIpAddr=hostbyteip;
	 rtpparams.DestPortBase=1;
	 rtpparams.dFrameRate=25;

	 if(rtp_init(&rtpparams)!=0)
	 {
	 printf("RTP init error.\n");
	 return 2;
	 }
...
unsigned long tm1=0,tm2=0,tm=0;
unsigned char res;

gettimeofday(&tv1,NULL);
tm1=tv1.tv_sec*1000+tv1.tv_usec/1000;

while(1)
{
	gettimeofday(&tv1,NULL);
	tm2=tv1.tv_sec*1000+tv1.tv_usec/1000;
	if((tm2-tm1)<FPSD)
	{
		usleep(FPSD-(tm2-tm1));
		continue;
	...
	if(fread(inputyuv,1,size,yuv_fp)!=size)
	{
		rewind(yuv_fp);
		continue;
	}
	num++;
	encode_frame(inputyuv,&cdbf);
	res=rtp_sendpackage(&cdbf);
	if(res!=0)
	{
		printf("RTP sending error.\n");
	}
	printf("\r frame number: %d",num);
}
